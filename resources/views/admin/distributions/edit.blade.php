...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] overflow-hidden">
        <div class="p-8 border-b border-gray-50">
            <h2 class="text-xl font-bold text-gray-800">Edit Data Penyaluran</h2>
            <p class="text-sm text-gray-500 mt-1">Perbarui data penggunaan dana wakaf.</p>
        </div>

        <form id="distribution-form" action="{{ route('admin.distributions.update', $distribution->id) }}" method="POST" enctype="multipart/form-data" class="mt-6 p-8" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="mb-6 form-group">
                ...
                <select name="campaign_id" required 
                    ...
                    @foreach($campaigns as $campaign)
                        <option value="{{ $campaign->id }}" {{ old('campaign_id', $distribution->campaign_id) == $campaign->id ? 'selected' : '' }}>
                            {{ $campaign->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6 form-group">
                ...
                <input type="text" name="title" value="{{ old('title', $distribution->title) }}" required 
            ...
            <div class="mb-6 form-group">
                ...
                <div class="flex items-center border rounded-xl transition-all overflow-hidden bg-white @error('amount') border-red-500 focus-within:border-red-500 focus-within:ring-red-500/20 @else border-gray-200 focus-within:border-primary focus-within:ring-primary/20 @enderror input-group-validate">
                    ...
                    <input type="hidden" name="amount" id="amount" value="{{ old('amount', $distribution->amount) }}">
                </div>
                <p class="mt-1 text-sm text-red-500 hidden validation-message"></p>
                @error('amount')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            ...
            <div class="mb-6 form-group">
                ...
                <input type="text" name="distributed_at" value="{{ old('distributed_at', $distribution->distributed_at) }}" required
            ...
            <div class="mb-6 form-group">
                ...
                <textarea name="description" required rows="5" 
                          ...
                          class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-primary/20 transition-all text-sm @error('description') border-red-500 focus:border-red-500 focus:ring-red-500/20 @enderror">{{ old('description', $distribution->description) }}</textarea>
                <p class="mt-1 text-sm text-red-500 hidden validation-message"></p>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-8 form-group">
                ...
                @if($distribution->documentation_url)
                    <div class="flex items-center gap-3 mb-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                        <div class="p-2 bg-white rounded-lg shadow-sm">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-gray-500"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>
                        </div>
                        <div class="flex-1">
                            <p class="text-sm font-medium text-gray-700">File Saat Ini</p>
                            <a href="{{ asset('storage/' . $distribution->documentation_url) }}" target="_blank" class="text-xs text-primary hover:underline">Lihat File</a>
                        </div>
                    </div>
                @endif
                ...
                <div class="relative group">
                    ...
                    <div id="upload-placeholder" class="border-2 border-dashed rounded-xl p-8 text-center transition-all group-hover:border-primary group-hover:bg-emerald-50/30 border-gray-200">
                        ...
                        <p class="text-sm font-medium text-gray-700 mb-1">Klik untuk ganti dokumentasi</p>
                        <p class="text-xs text-gray-400">Format GIF, JPG, PNG, PDF (Max. 5MB). Biarkan kosong jika tidak berubah.</p>
                    </div>
