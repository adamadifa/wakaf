...
@section('title', 'Verifikasi Zakat')
...
<div class="mb-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
             <h1 class="text-2xl font-bold text-gray-800">Verifikasi Zakat</h1>
             <p class="text-gray-500 mt-1">Kelola dan verifikasi transaksi zakat masuk.</p>
        </div>
    </div>

    <!-- Multi-Filter Section -->
    <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        <form action="{{ route('admin.zakat-transactions.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-12 gap-4">
                ...
                <div class="lg:col-span-4">
                    ...
                    <div class="relative">
                        <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari invoice / nama muzakki..." 
                ...
                <div class="lg:col-span-2">
                    <label class="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wider">Jenis Zakat</label>
                    <select name="zakat_type_id" class="select2 w-full px-3 py-2 bg-gray-50 border-gray-100 rounded-xl focus:bg-white focus:border-primary focus:ring-primary/20 transition-all text-sm appearance-none">
                        <option value="">Semua</option>
                        @foreach($zakatTypes as $type)
                            <option value="{{ $type->id }}" {{ request('zakat_type_id') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="lg:col-span-2 flex items-end gap-2">
                    ...
                    @if(request()->anyFilled(['q', 'zakat_type_id', 'status', 'start_date', 'end_date']))
                    <a href="{{ route('admin.zakat-transactions.index') }}" class="px-3 h-[42px] bg-red-50 text-red-500 rounded-xl hover:bg-red-100 transition-colors flex items-center justify-center" title="Reset Filter">
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-gray-50/50 border-b border-gray-100">
                    ...
                    <th class="text-left py-4 px-6 text-sm font-semibold text-gray-600">Muzakki (Donatur)</th>
                    <th class="text-left py-4 px-6 text-sm font-semibold text-gray-600">Jenis Zakat</th>
                    <th class="text-left py-4 px-6 text-sm font-semibold text-gray-600">Jumlah</th>
                    <th class="text-left py-4 px-6 text-sm font-semibold text-gray-600">Metode Bayar</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors group">
                    <td class="py-4 px-6">
                        <span class="font-mono text-xs font-semibold text-gray-500">{{ $transaction->invoice_number }}</span>
                    </td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6">
                        <div class="text-sm text-gray-600">{{ $transaction->zakatType->name }}</div>
                    </td>
                    <td class="py-4 px-6">
                        <div class="font-bold text-gray-800">Rp {{ number_format($transaction->total_transfer, 0, ',', '.') }}</div>
                        @if($transaction->unique_code > 0)
                        <div class="text-xs text-gray-500">
                            (Kode Unik: <span class="text-gray-700 font-semibold">{{ $transaction->unique_code }}</span>)
                        </div>
                        @else
                        <div class="text-xs text-gray-500">
                            (Tanpa Kode Unik)
                    ...
                    <td class="py-4 px-6">
                        <div class="text-sm text-gray-600">{{ $transaction->paymentMethod->bank_name ?? '-' }}</div>
                        <div class="text-xs text-gray-500">{{ $transaction->paymentMethod->account_number ?? '' }}</div>
                    </td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center justify-end gap-2 text-right">
                            <a href="{{ route('admin.zakat-transactions.show', $transaction->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-50 text-gray-400 hover:text-primary hover:bg-emerald-50 transition-all border border-gray-100" title="Detail & Verifikasi">
