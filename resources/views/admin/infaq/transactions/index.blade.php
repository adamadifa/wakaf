...
@section('title', 'Verifikasi Infaq')
...
<div class="mb-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
             <h1 class="text-2xl font-bold text-gray-800">Verifikasi Infaq</h1>
             <p class="text-gray-500 mt-1">Kelola dan verifikasi transaksi infaq/sedekah masuk.</p>
        </div>
    </div>

    <!-- Multi-Filter Section -->
    <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        <form action="{{ route('admin.infaq-transactions.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-12 gap-4">
                ...
                <div class="lg:col-span-2">
                    <label class="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wider">Program Infaq</label>
                    <select name="infaq_category_id" class="select2 w-full px-3 py-2 bg-gray-50 border-gray-100 rounded-xl focus:bg-white focus:border-primary focus:ring-primary/20 transition-all text-sm appearance-none">
                        <option value="">Semua</option>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ request('infaq_category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="lg:col-span-2 flex items-end gap-2">
                    ...
                    @if(request()->anyFilled(['q', 'infaq_category_id', 'status', 'start_date', 'end_date']))
                    <a href="{{ route('admin.infaq-transactions.index') }}" class="px-3 h-[42px] bg-red-50 text-red-500 rounded-xl hover:bg-red-100 transition-colors flex items-center justify-center" title="Reset Filter">
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td class="py-4 px-6">
                        <div class="text-sm text-gray-600">{{ $transaction->infaqCategory->name }}</div>
                    </td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center justify-end gap-2 text-right">
                            <a href="{{ route('admin.infaq-transactions.show', $transaction->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-50 text-gray-400 hover:text-primary hover:bg-emerald-50 transition-all border border-gray-100" title="Detail & Verifikasi">
