...
<!-- Header Back -->
<div class="mb-6">
    <a href="{{ route('admin.infaq-transactions.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-primary transition-colors">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
        Kembali ke Verifikasi Infaq
    </a>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <!-- Donatur Info -->
        <div class="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] p-6">
            <h2 class="text-lg font-bold text-gray-800 mb-4">Detail Donatur</h2>
            ...
            <div class="flex items-start gap-4 p-4 border rounded-xl bg-gray-50/50">
                <div class="w-16 h-16 rounded-lg overflow-hidden shrink-0 flex items-center justify-center bg-gray-200">
                     <i class="{{ $transaction->infaqCategory->icon ?? 'ti-package' }} text-2xl text-gray-500"></i>
                </div>
                <div>
                    <h4 class="font-semibold text-gray-900 line-clamp-1">{{ $transaction->infaqCategory->name }}</h4>
                    <p class="text-xs text-gray-500 mt-1">{{ Str::limit($transaction->infaqCategory->description, 60) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] p-6">
            ...
            <div class="flex flex-col gap-3">
                <form action="{{ route('admin.infaq-transactions.update', $transaction->id) }}" method="POST" class="w-full">
                    @csrf
                    @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] p-6">
            ...
            <p class="text-sm text-gray-500 mb-6">Transaksi ini sudah diverifikasi.</p>
            
            <form action="{{ route('admin.infaq-transactions.cancel', $transaction->id) }}" method="POST" class="w-full">
                @csrf
                @method('PUT')
                <button type="submit" class="w-full py-2.5 bg-white text-red-600 border border-red-200 rounded-xl font-semibold hover:bg-red-50 transition-colors" onclick="return confirm('Apakah Anda yakin ingin membatalkan verifikasi ini? Status akan kembali menjadi PENDING.')">
