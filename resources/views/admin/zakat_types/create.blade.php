...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-8 pl-1">
        ...
        <p class="text-gray-500 mt-1">Tambahkan jenis zakat baru untuk ditampilkan di halaman depan.</p>
    </div>

    <form action="{{ route('admin.zakat-types.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 shadow-sm space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="category" required 
                            ...
                            <option value="mal" {{ old('category') == 'mal' ? 'selected' : '' }}>Zakat Mal (Harta)</option>
                            <option value="fitrah" {{ old('category') == 'fitrah' ? 'selected' : '' }}>Zakat Fitrah</option>
                        </select>
            ...
            <div>
                ...
                <textarea name="description" rows="4" placeholder="Keterangan singkat tentang jenis zakat ini..." 
                          class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-sm outline-none resize-none">{{ old('description') }}</textarea>
            </div>
