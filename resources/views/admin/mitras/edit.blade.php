...
<div class="mb-8">
    ...
    <div class="max-w-xl">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-6">Edit Mitra</h2>
            ...
            <form action="{{ route('admin.mitras.update', $mitra->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $mitra->name) }}" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-primary focus:ring-primary text-sm" placeholder="Masukkan nama mitra" required>
                        @error('name')
                    ...
                    <!-- Current Logo -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Logo Saat Ini</label>
                        <div class="bg-gray-50 rounded-xl p-4 flex items-center justify-center">
                            <img src="{{ asset('storage/' . $mitra->logo) }}" alt="{{ $mitra->name }}" class="max-h-20 object-contain">
                        </div>
                    </div>

                    <!-- New Logo Upload -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Ganti Logo <span class="text-gray-400 font-normal">(opsional)</span></label>
                        <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:bg-gray-50 transition-colors cursor-pointer" onclick="document.getElementById('logo-input').click()">
                            ...
                            <div id="logo-placeholder">
                                ...
                                <p class="text-sm font-semibold text-gray-600">Upload Logo Baru</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $mitra->sort_order) }}" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-primary focus:ring-primary text-sm" placeholder="0">
                        <p class="text-xs text-gray-400 mt-1">Semakin kecil angka, semakin atas posisinya.</p>
                    </div>

                    <!-- Active -->
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" id="is_active" {{ $mitra->is_active ? 'checked' : '' }} class="rounded border-gray-300 text-primary focus:ring-primary">
